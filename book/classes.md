...
The most accurate way to characterize a system's design would be to wait for it to be decommissioned, then assess the costs over the . Was it increasingly slower to introduce new features as the system grew and aged? Did the number of defects increase? (Never mind that measuring some of these characteristics might be near impossible. How, for example, do you compare the size of one feature to another?)  [[ maybe reference Capers Jones, "Programming Productivity"? or something from Brooks, "no silver bullet"? ]])

That final assessment wouldn't be terribly useful to any of the system's developers--they've all now retired or moved onto other gigs. But we can carry that assessment forward, which is what we've done across seven decades of software development. We've conducted research and we've gathered insights. And we've continued to extrapolate those learnings into larger and newer contexts.
...
Pioneers Maurice Wilkes and David Wheeler were inspired by ideas found in the seminal article, and implemented the concept of subroutines in the EDSAC computer in 1951. In short, they believed that the subroutines would lead to less costly software development. They didn't wait for research (though it did come, slowly, across later decades).

Since the subroutine, we've extrapolated the claimed value of modularization, originally brought about in the form of subroutines, to larger contexts. We organize collections of subroutines&mdash;functions or methods&mdash;into modules or classes. Developers since have noted the positive or negative outcomes of various choices at this larger level, and many have formulated various heuristics as a result.
